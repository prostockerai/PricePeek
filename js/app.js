...
async function performSearch(forceRefresh = false) {
    ...
    document.getElementById('loadingText').textContent = 'Fetching live prices from Daraz, StarTech, Ryans, Pickaboo...';
    ...
    try {
        ...
        if (products.length === 0) {
            document.getElementById('productGrid').innerHTML = `
                <div class="error-state" style="grid-column: 1/-1;">
                    ...
                    ${errors.length > 0 ? `<p style="margin-top: 8px; font-size: 12px;">Note: Some marketplaces may be temporarily unavailable</p>` : ''}
            ...
            applyFiltersAndSort();
            showNotification(`Found ${products.length} products across ${new Set(products.map(p => p.marketplace)).size} marketplaces`, 'success');
        }
    } catch (error) {
        ...
        document.getElementById('productGrid').innerHTML = `
            <div class="error-state" style="grid-column: 1/-1;">
                ...
                <button class="retry-btn" onclick="performSearch(true)">🔄 Retry</button>
            </div>
        `;
    ...
    if (products.length === 0) {
        grid.innerHTML = `
            <div style="grid-column: 1/-1; text-align: center; padding: 40px;">
                <p style="color: #9CA3AF;">No products match current filters</p>
            </div>
        `;
        return;
    }
    ...
    grid.innerHTML = products.map(product => {
        ...
        return `
            <div class="product-card ${isCheapest ? 'best-choice' : ''}" data-id="${product.id}">
                ...
                <span class="marketplace-badge ${marketplaceClass}">
                    🏪 ${product.marketplace}
                    ${product.isOfficial ? '✅ Official' : ''}
                </span>
                
                <div class="product-image-container">
                    ${product.image ? 
                        `<img src="${product.image}" alt="${product.name}" onerror="this.parentElement.innerHTML='<span class=\\'product-image-placeholder\\'>📦</span>'">` :
                ...
                ${product.coupons && product.coupons.length > 0 ? `
                    <div class="coupon-row">
                        ${product.coupons.map(c => `
                            <span class="coupon-chip" onclick="copyToClipboard('${c.code}')" title="Click to copy">
                                🎫 ${c.code}
                ...
                ${product.cashback && product.cashback.length > 0 ? `
                    <div class="coupon-row">
                        ${product.cashback.map(c => `
                            <span class="cashback-chip">
                                💰 ${c.provider} ${c.percentage}% (Max ৳${c.maxAmount})
                            </span>
                        `).join('')}
                    </div>
                ` : ''}
                ...
                <div class="card-actions">
                    <a href="${product.url || '#'}" target="_blank" class="btn-visit btn-primary" 
                       onclick="trackClick('${product.marketplace}', '${product.name}')">
                        Visit Store →
                    </a>
                ...
                ${product.scrapedAt ? `
                    <div style="font-size: 10px; color: #9CA3AF; margin-top: 8px; text-align: right;">
                        Updated: ${new Date(product.scrapedAt).toLocaleTimeString()}
                    </div>
                ` : ''}
            </div>
        `;
...
function updateBestDeal(products) {
    ...
    // Find best deal based on discount + coupons + cashback
    const bestDeal = inStock.reduce((best, current) => {
        const currentScore = (current.discount || 0) + 
            (current.coupons?.length || 0) * 3 + 
            (current.cashback?.length || 0) * 2;
        const bestScore = (best.discount || 0) + 
            (best.coupons?.length || 0) * 3 + 
            (best.cashback?.length || 0) * 2;
    ...
    if (bestDeal) {
        ...
        const savings = bestDeal.originalPrice ? 
            bestDeal.originalPrice - bestDeal.price : 0;
...
function updateWishlistCount() {
    document.getElementById('wishlist-count').textContent = APP_STATE.wishlist.length;
}
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <thead>
                    <tr>
                        ...
                        ${APP_STATE.comparisonList.map(p => `
                            <th style="padding: 12px; border-bottom: 2px solid #E5E7EB; text-align: center;">
                                <div style="font-weight: 600;">${p.marketplace}</div>
                                <div style="font-size: 11px; color: #6B7280;">${p.name?.substring(0, 30)}...</div>
                            </th>
                        `).join('')}
                    </tr>
                </thead>
...
function createComparisonRow(label, valueFn) {
    return `
        <tr>
            <td style="padding: 10px; border-bottom: 1px solid #F3F4F6; font-weight: 600;">${label}</td>
            ${APP_STATE.comparisonList.map(p => `
...
// ============ DEALS PAGE ============
async function showDealsPage(forceRefresh = false) {
    document.getElementById('loadingSpinner').classList.add('active');
    document.getElementById('loadingText').textContent = 'Fetching best deals from all marketplaces...';
    document.getElementById('resultsSection').classList.add('active');
    ...
    uniqueProducts.sort((a, b) => (b.discount || 0) - (a.discount || 0));
    
    APP_STATE.allProducts = uniqueProducts.slice(0, 50);
    APP_STATE.currentFilter = 'all';
