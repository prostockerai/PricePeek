...
class DarazScraper extends BaseScraper {
  ...
  async search(query) {
    ...
    try {
      ...
      // 2025 আপডেটেড সিলেক্টর (ডারাজের বর্তমান স্ট্রাকচার)
      $('[data-qa-locator="product-item"], .Bm3ON, .gridItem--Yd0sa, .buTCk, .RfADt').each((i, el) => {
        ...
        try {
          ...
          const originalPrice = this.parsePrice(
            $el.find('.originalPrice--aY4fQ, .crossPrice, .c13VH6 + .c13VH6').text()
          );
